...
export class DataApprovalStatus extends PureComponent {
    ...
    fetchApprovalStatus = () => {
        ...
        const url = `dataApprovals?ds=${
            dataSet.id
        }&pe=${periodId}&ou=${organisationUnitId}`
        d2.Api.getApi()
            .get(url)
    ...
    confirmDialogActions = () => [
        ...
        <Button color="primary" onClick={this.handlerForCurrentAction()}>
    ...
    render() {
        return (
            <div>
                <span style={styles.notificationBar}>
                    {`${i18n.t(i18nKeys.dataApproval.approvalStatusLabel)}:
                    ${i18n.t(
                        i18nKeys.dataApproval.statusNotifications[
                            this.state.status
                        ]
                    )}`}
                </span>
                <div id="approvalActions">
                    {isReadyForApprove(this.state) && (
                        <Button
                            ...
                            raised
                            color="primary"
                            onClick={this.showConfirmAction(
                                DataApprovalActionsEnum.APPROVE
                            )}
                        >
                    ...
                    {isReadyForUnapprove(this.state) && (
                        <Button
                            ...
                            color="primary"
                            onClick={this.showConfirmAction(
                                DataApprovalActionsEnum.UNAPPROVE
                            )}
                        >
                            {i18n.t(
                                i18nKeys.dataApproval.buttonLabels.unapprove
                            )}
                        </Button>
                    )}
                    {isReadyForAcceptance(this.state) && (
                        <Button
                            ...
                            color="primary"
                            onClick={this.showConfirmAction(
                                DataApprovalActionsEnum.ACCEPT
                            )}
                        >
                    ...
                    {isReadyForUnacceptance(this.state) && (
                        <Button
                            ...
                            color="primary"
                            onClick={this.showConfirmAction(
                                DataApprovalActionsEnum.UNACCEPT
                            )}
                        >
                            {i18n.t(
                                i18nKeys.dataApproval.buttonLabels.unaccept
                            )}
                        </Button>
                    )}
                </div>
                ...
                >
                    {i18n.t(
                        i18nKeys.dataApproval.confirmActionNotifications[
                            this.state.action
                        ]
                    )}
                </Dialog>
...
export default props => (
    <AppContext.Consumer>
        {appContext => <DataApprovalStatus d2={appContext.d2} {...props} />}
    </AppContext.Consumer>
)
