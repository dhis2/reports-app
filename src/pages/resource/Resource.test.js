...
const documents = [
    { id: "hKYLLpNinZR", external: true, displayName: "DHIS 2 Home Page", url: "http://dhis2.org" },
    { id: "V8emVumNG4D", external: true, displayName: "DHIS 2 Launchpad site", url: "https://launchpad.net/dhis2" },
    { id: "bhjS25ZgCxD", external: true, displayName: "HISP EA Home Page", url: "http://hispea.com" },
    { id: "LMb4hjq8xWZ", external: false, displayName: "Immunization health data report", url: "data.xls" }
];
...
describe('Test <Resource /> rendering:', () => {
    ...
    it('Should show correct title.', () =>{
        ...
        expect(wrapper.find('h1').text()).toBe(`${i18nKeys.resource.header}<PageHelper />`);
    });
    ...
    it('Should render a "Resource" table with correct columns.' , () => {
        ...
        expect(wrapper.find('#no-resource-find-message-id').props().style.display).toBe('none');
    });
    ...
    it('Should render a "Resource" table with correct rows.' , () => {
        ...
        expect(wrapper.find(Table).props().rows[0].displayName).toBe(documents[0].displayName);
        expect(wrapper.find(Table).props().rows[1].displayName).toBe(documents[1].displayName);
        expect(wrapper.find(Table).props().rows[2].displayName).toBe(documents[2].displayName);
        expect(wrapper.find(Table).props().rows[3].displayName).toBe(documents[3].displayName);
        expect(wrapper.find('#no-resource-find-message-id').props().style.display).toBe('none');
    });
...
describe('Test <Resource /> actions:', () => {
    ...
    it('Should call addNewResource action when Add button clicked.', () => {
        const spy = spyOn(Resource.prototype, 'addNewResource').and.callThrough();
        const wrapper = ownShallow();
    ...
    it('Should update state properly for "View Resource" menu action.', () => {
        ...
        expect(wrapper.state().selectedResource.displayName).toBe('documentName');
    ...
    it('Should update state properly for "Sharing settings" menu action.', () => {
        const wrapper = ownShallow();
        const args = { displayName: 'nameSharingSettings', id: 'idSharingSettings'};
        wrapper.instance().sharingSettings(args);
        expect(wrapper.state().selectedResource.displayName).toBe('nameSharingSettings');
