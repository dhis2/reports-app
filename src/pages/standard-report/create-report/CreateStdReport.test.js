...
describe('Test <CreateStdReport /> rendering:', () => {
    ...
    it('Should have correct title.', () => {
        expect(dialog.props().title).toBe(`${i18nKeys.standardReport.createReportTitle}`);
    });
    ...
    it('Should display "Cancel", "Download as Excel" and "Get Report" actions.', () => {
        ...
        expect(actions.find(Button).at(1).text()).toBe(`${i18nKeys.buttons.downloadAsExcel}`);
        expect(actions.find(Button).at(2).text()).toBe(`${i18nKeys.buttons.getReport}`);
    });
});
...
describe('Test <CreateStdReport /> actions:', () => {
    ...
    it('Should call loadReportParams when selected different report.', () => {
        wrapper.instance().loadReportParams = jest.fn();
        wrapper.instance().componentWillReceiveProps({ selectedReport: { id: 'newId' } });
        expect(wrapper.instance().loadReportParams).toHaveBeenCalled();
    });

    it('Should call onChangePeriod when selected period changes.', () => {
        wrapper.instance().onChangePeriod = jest.fn();
        wrapper.setState( { periods, params, selectedPeriod: '201807', selectedOrgUnitId: 'OrgUnit' } );
    ...
    it('Should call getReport when get report button clicked.', () => {
        ...
        wrapper.setState( { periods, params, selectedPeriod: '201807', selectedOrgUnitId: 'OrgUnit' } );
        const actions = mount(<MuiThemeProvider><span>{wrapper.find(Dialog).props().actions}</span></MuiThemeProvider>);
        actions.find(Button).at(2).find('button').simulate('click');
