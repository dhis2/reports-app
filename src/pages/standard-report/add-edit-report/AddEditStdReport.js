...
export class AddEditStdReport extends PureComponent {
    ...
    componentWillReceiveProps(nextProps) {
        if (
            ...
            nextProps.selectedReport.id !== this.state.report.id &&
            !this.state.loading
        ) {
    ...
    onChangeCacheStrategy = strategy => {
        this.setState({
            report: { ...this.state.report, cacheStrategy: strategy.id },
        })
    }

    onChangeReportTable = table => {
        this.setState({
            report: { ...this.state.report, reportTable: { id: table.id } },
        })
    }
    ...
    onChangeCheck = event => {
        this.setState({
            report: {
                ...this.state.report,
                relativePeriods: {
                    ...this.state.report.relativePeriods,
    ...
    onChangeFileTemplate = event => {
        ...
        if (event.target.files[0]) {
            ...
            this.setState({
                selectedFileToUpload: null,
                report: { ...this.state.report, designContent: null },
            })
        }
        ...
        this.setState({
            report: {
                ...
                reportParams: {
                    ...
                    paramReportingPeriod: event.target.checked,
                },
            },
        })
    }
    ...
    onChangeCbOrgUnit = event => {
        this.setState({
            report: {
                ...
                reportParams: {
                    ...this.state.report.reportParams,
                    paramOrganisationUnit: event.target.checked,
                },
            },
        })
    }
        ...
        if (api) {
            ...
            api.get(url)
                .then(response => {
                    if (response) {
                        this.setState({
                            reportTables: [NONE, ...response.reportTables],
                        })
                    }
                })
                .catch(error => {
        ...
        if (this.ifFormValid()) {
            ...
            if (api) {
                ...
                if (this.state.report.id) {
                    ...
                    api.update(url, this.state.report)
                        .then(response => {
                            if (response) {
                                this.close(true)
                            }
                    ...
                    api.post(REPORTS_ENDPOINT, this.state.report)
                        .then(response => {
                            if (response) {
                                this.close(true)
                            }
    ...
    validateGenericFields = () =>
        !!(this.state.report.name && this.state.report.designContent)
    ...
    render() {
        ...
        return (
            ...
            >
                <div
                    id={'add-edit-std-report-form-id'}
                    style={styles.dialogContentContainer}
                >
                    <span
                        id={'display-right-message-id'}
                        className={'row'}
                            ...
                            <SelectField
                                ...
                                value={
                                    this.state.report.reportTable
                                        ? this.state.report.reportTable.id
                                        : NONE.id
                                }
                    ...
                    {/* relative periods  */}
                    <div
                        id={'relative-periods-id'}
                        className={'row'}
                        style={this.showSection()}
                    >
                        <div
                            className={'col-xs-12'}
                            style={styles.sectionTitle}
                        ...
                        <div
                            ...
                            className="row"
                            style={styles.relativePeriodsRow}
                        >
                            {relativePeriods.map(relativePeriod => (
                                <div
                                    key={relativePeriod.label}
