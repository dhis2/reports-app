...
defineSupportCode(({ Given, When, Then }) => {
    ...
    Then(/^I click add resource button$/, () => {
        ...
        this.numberOfItems = browser
            .element('.data-table-pager > ul > li')
            .getText()
            .split('/')[1]
        browser
            .element('#add-resource-btn-container-id button')
            .waitForVisible(DEFAULT_WAIT_TIME)
        browser.element('#add-resource-btn-container-id button').click()
        browser
            .element('#add-edit-resource-form-id')
            .waitForVisible(DEFAULT_WAIT_TIME)
    })
    ...
    Then(/^a new resource was created$/, () => {
        ...
        expect(parseInt(newNumberOfItems, 10)).to.equal(
            parseInt(this.numberOfItems, 10) + 1
        )
    })
    ...
    Then(/^I click more options icon in the resource list item$/, () => {
        ...
        this.selectedResourceName = browser
    ...
    Then(/^button to add resource is displayed$/, () => {
        browser
            .element('#add-resource-btn-container-id')
            .waitForVisible(DEFAULT_WAIT_TIME)
    })
    ...
    Then(/^the new resource was not created$/, () => {
        const newNumberOfItems = browser
            ...
            .split('/')[1]
        expect(parseInt(newNumberOfItems, 10)).to.equal(
            parseInt(this.numberOfItems, 10)
        )
    })
    ...
    Then(
        /^I click more options icon in the resource list item configured with file$/,
        () => {
            browser.element('.d2-ui-table').waitForVisible(DEFAULT_WAIT_TIME)
            resource.getContextMenuBtnForResourceFromList(0).click()
            browser.pause(DEFAULT_WAIT_TIME)
    ...
    // *********************************************************
    Then(
        /^I click more options icon in the resource list item configured with html$/,
        () => {
            browser.element('.d2-ui-table').waitForVisible(DEFAULT_WAIT_TIME)
    ...
    Then(/^the resource is updated$/, () => {
        expect(
            browser
                ...
                .value[0].element('<div>')
                .getText()
        ).to.equal('0 Test - File Resource Edited')
    })
    ...
    Then(/^form with selected resource name is displayed$/, () => {
        ...
        expect(
            browser
                ...
                .getText()
        ).to.equal(this.selectedResourceName)
    })
    ...
    Then(/^I change resource share rules/, () => {
        ...
        this.existingRules = resource.processAccessRulesNumber(
            this.existingRules
        )
